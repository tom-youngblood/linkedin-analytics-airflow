...
def main():
    ...
    # Get the encoded key from environment variable using Airflow-aware utility
    try:
        ...
        logger.error(f"Failed to get SERVICE_ACCOUNT_KEY: {e}")
        raise
    ...
    try:
        ...
        links = pd.DataFrame(gc.open(sheet_name).worksheet(worksheet_name).get_all_values(), columns=['link', 'post', 'id'])
